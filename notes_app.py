...
def _parse_excel(uploaded_file):
    """Lit un fichier Excel — robuste avec gestion d'erreurs complète."""
    import openpyxl
    import warnings

    # Lire le contenu en mémoire pour éviter les problèmes de stream
    # Accepte à la fois un fichier uploadé et un BytesIO
    if hasattr(uploaded_file, 'read'):
        raw = uploaded_file.read()
        file_bytes = io.BytesIO(raw if isinstance(raw, bytes) else raw.encode())
    else:
    ...
    if header_row_idx is not None:
        ...
        for row in rows[header_row_idx + 2:]:
            try:
                if (len(row) > 3 and row[3] and
                    isinstance(row[3], str) and len(str(row[3]).strip()) > 1):
                    nom = str(row[3]).strip()
                    ...
                    if any(v is not None for v in [d1, d2, d3]):
                        data.append({"Etudiant": nom, "Devoir1": d1,
                                     "Devoir2": d2, "Devoir3": d3, "Activites": act})
            except Exception:
                continue
        if data:
...
if uploaded:
    try:
        # Lire le contenu complet en mémoire d'abord
        ...
        if uploaded.name.endswith(".csv"):
            ...
            # Créer un objet file-like depuis les bytes
            uploaded_bytes = io.BytesIO(file_content)
            uploaded_bytes.name = uploaded.name
            df = _parse_excel(uploaded_bytes)
        ...
        if missing:
            ...
            act_stat_html = f"""
                <div class="stat-box gold">
                    <div class="stat-label">Moy. Activités</div>
                    <div class="stat-value">{moy_act:.2f}</div>
                </div>""" if moy_act is not None else ""
            ...
            <div class="stat-grid">
                <div class="stat-box">
                    <div class="stat-label">Moy. Devoir 1</div>
                    <div class="stat-value">{moy_d1:.2f}</div>
                </div>
                <div class="stat-box">
                    <div class="stat-label">Moy. Devoir 2</div>
                    <div class="stat-value">{moy_d2:.2f}</div>
                </div>
                <div class="stat-box">
                    <div class="stat-label">Moy. Devoir 3</div>
                    <div class="stat-value">{moy_d3:.2f}</div>
                </div>
                {act_stat_html}
            </div>
            """, unsafe_allow_html=True)
