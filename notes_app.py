            ...
            for j in range(i + 2, len(df_raw)):
                ...
                try:
                    ...
                    if nom and isinstance(nom, str) and len(nom.strip()) > 1:
                        ...
                        if any(pd.notna(v) for v in [d1, d2, d3]):
...
if uploaded:
    try:
        ...
        if fname.endswith(".csv"):
            try:
                df_raw = pd.read_csv(io.BytesIO(file_content), header=None, encoding="utf-8")
            except Exception:
                df_raw = pd.read_csv(io.BytesIO(file_content), header=None, encoding="latin-1")
            df_raw.columns = range(len(df_raw.columns))
            df = _parse_standard(df_raw)

        elif fname.endswith(".xls"):
            df_raw = pd.read_excel(io.BytesIO(file_content), header=None, engine="xlrd")
            df = _detect_format(df_raw)

        else:  # .xlsx
            # Essayer d'abord la feuille NotesCC (format MASSAR)
            try:
                ...
                sheet = "NotesCC" if "NotesCC" in wb.sheetnames else wb.sheetnames[0]
