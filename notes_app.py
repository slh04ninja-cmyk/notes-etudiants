...
if uploaded:
    with st.spinner("Chargement du fichier en cours..."):
        try:
            file_content = uploaded.getvalue()  # Plus stable que .read() sur mobile
            ...
            else:  # .xlsx — pandas uniquement, sans openpyxl direct
                import warnings
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore")
                    # Lire toutes les feuilles et choisir la bonne
        ...
        if missing:
            ...
            html_row2 = (
                ...
                + '<div class="stat-value">' + f"{moy_d3:.2f}" + '</div></div>'
                + act_box
                + '</div>'
            )
