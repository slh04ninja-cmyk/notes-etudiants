...
sample_data.to_excel(sample_buf, index=False)
sample_buf.seek(0)
st.download_button("📥 Télécharger fichier exemple", data=sample_buf,
                   file_name="exemple_notes.xlsx",
...
if uploaded:
    try:
        ...
        for c in df.columns:
            ...
            elif any(x in cl for x in ["d1","devoir1","devoir 1","note1","ds1"]): col_map[c] = "Devoir1"
            elif any(x in cl for x in ["d2","devoir2","devoir 2","note2","ds2"]): col_map[c] = "Devoir2"
            elif any(x in cl for x in ["d3","devoir3","devoir 3","note3","ds3"]): col_map[c] = "Devoir3"
        df = df.rename(columns=col_map)

        required = ["Etudiant","Devoir1","Devoir2","Devoir3"]
        missing  = [c for c in required if c not in df.columns]
        if missing:
            st.error(f"❌ Colonnes manquantes : {missing}. Vérifiez le format du fichier.")
        else:
            ...
            with tab3:
                ...
                df_rank["Progression"] = df_rank["Progression"].apply(
                    lambda x: f"+{x:.1f}" if x > 0 else f"{x:.1f}")
                df_rank["Moyenne"] = df_rank["Moyenne"].round(2)

                display_df = df_rank[["Rang","Etudiant","Devoir1","Devoir2","Devoir3",
                                      "Moyenne","Mention","Tendance","Progression"]]
                ...
                medals = ["🥇","🥈","🥉"]
                for idx, (medal, col) in enumerate(zip(medals, top3_cols)):
                    if idx < len(df_rank):
                        r = df_rank.iloc[idx]
                        mention, _ = get_mention(float(r["Moyenne"]))
                        col.metric(f"{medal} {r['Etudiant']}", f"{r['Moyenne']}/20", mention)
