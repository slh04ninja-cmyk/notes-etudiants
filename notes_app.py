...
def get_mention(moyenne):
    if moyenne >= 16:   return "Tres Bien",  "mention-excellent"
    ...
    else:               return "Echec",      "mention-insuffisant"
...
def _ar(text):
    ...
    try:
        ...
        # Fallback sans librairie : inverser les mots pour RTL
        t = str(text).strip()
...
def make_pdf_bulletin(df_result, nom_classe=""):
    ...
    # ── Police Arabic ──
    # Amiri-Regular.ttf doit être dans le repo GitHub (même dossier que notes_app.py)
    arabic_font      = "Helvetica"
    arabic_font_bold = "Helvetica-Bold"

    try:
        import os as _os
        # Chercher la police dans le dossier du script (repo GitHub)
        script_dir  = _os.path.dirname(_os.path.abspath(__file__)) if "__file__" in dir() else "/mount/src/notes-etudiants"
        amiri_path  = _os.path.join(script_dir, "Amiri-Regular.ttf")

        # Fallback: dossier courant
        if not _os.path.exists(amiri_path):
            amiri_path = "Amiri-Regular.ttf"

        if _os.path.exists(amiri_path):
    ...
    # ── Header ──
    classe_s = ParagraphStyle("cl", fontSize=13, fontName="Helvetica-Bold",
                               textColor=colors.white)
    left_cell  = Paragraph(nom_classe if nom_classe else "", classe_s)
    right_cell = Paragraph("Bulletin de Notes — Suivi de Progression", title_s)
    ht = Table([[left_cell, right_cell]], colWidths=[8*cm, 19*cm])
    ...
    # ── Détection nb devoirs et activités ──
    has_d3_pdf  = "Devoir3" in df_result.columns and df_result["Devoir3"].notna().any()
    has_act_pdf = "Activites" in df_result.columns and df_result["Activites"].notna().any()
    prog_label  = "Prog D1-D3" if has_d3_pdf else "Prog D1-D2"

    # ── En-têtes tableau selon nb devoirs ──
    if has_d3_pdf and has_act_pdf:
        headers = ["N°", "Nom Etudiant", "D1/20", "D2/20", "D3/20", "Act./20", "Moy./20", "Mention", "Tendance", prog_label]
        col_w   = [0.8*cm, 5.5*cm, 1.8*cm, 1.8*cm, 1.8*cm, 1.8*cm, 2*cm, 2.8*cm, 2.3*cm, 2*cm]
    elif has_d3_pdf and not has_act_pdf:
        headers = ["N°", "Nom Etudiant", "D1/20", "D2/20", "D3/20", "Moy./20", "Mention", "Tendance", prog_label]
        col_w   = [0.8*cm, 6*cm, 2*cm, 2*cm, 2*cm, 2.2*cm, 3*cm, 2.5*cm, 2.5*cm]
    elif not has_d3_pdf and has_act_pdf:
        headers = ["N°", "Nom Etudiant", "D1/20", "D2/20", "Act./20", "Moy./20", "Mention", "Tendance", prog_label]
        col_w   = [0.8*cm, 6*cm, 2*cm, 2*cm, 2*cm, 2.2*cm, 3*cm, 2.5*cm, 2.5*cm]
    else:
        headers = ["N°", "Nom Etudiant", "D1/20", "D2/20", "Moy./20", "Mention", "Tendance", prog_label]
        col_w   = [0.8*cm, 7*cm, 2*cm, 2*cm, 2.5*cm, 3*cm, 2.5*cm, 2.5*cm]
    ...
    for i, row in enumerate(df_sorted.itertuples(), 1):
        ...
        d1_s = "{:.2f}".format(row.Devoir1) if pd.notna(row.Devoir1) else "-"
        d2_s = "{:.2f}".format(row.Devoir2) if pd.notna(row.Devoir2) else "-"

        # D3
        if has_d3_pdf:
            d3_v = getattr(row, "Devoir3", None)
            d3_s = "{:.2f}".format(d3_v) if d3_v is not None and pd.notna(d3_v) else "-"
        # Act
        if has_act_pdf:
            act_v = getattr(row, "Activites", None)
            act_s = "{:.2f}".format(act_v) if act_v is not None and pd.notna(act_v) else "-"

        # Progression
        try:
            if has_d3_pdf and d3_v is not None and pd.notna(d3_v):
                prog = float(d3_v) - float(row.Devoir1)
            else:
                prog = float(row.Devoir2) - float(row.Devoir1)
            prog_s = ("+{:.1f}".format(prog)) if prog > 0 else "{:.1f}".format(prog)
        except Exception:
            prog_s = "-"
        ...
        try:
            if has_d3_pdf and d3_v is not None and pd.notna(d3_v):
                diff = float(d3_v) - float(row.Devoir1)
            else:
                diff = float(row.Devoir2) - float(row.Devoir1)
            trend = "En progres" if diff > 1 else ("En baisse" if diff < -1 else "Stable")
        except Exception:
            trend = "Stable"

        # Construire la ligne selon le cas
        if has_d3_pdf and has_act_pdf:
            tdata.append([str(i), nom_ar, d1_s, d2_s, d3_s, act_s, "{:.2f}".format(row.Moyenne), mention, trend, prog_s])
        elif has_d3_pdf and not has_act_pdf:
            tdata.append([str(i), nom_ar, d1_s, d2_s, d3_s, "{:.2f}".format(row.Moyenne), mention, trend, prog_s])
        elif not has_d3_pdf and has_act_pdf:
            tdata.append([str(i), nom_ar, d1_s, d2_s, act_s, "{:.2f}".format(row.Moyenne), mention, trend, prog_s])
        else:
            tdata.append([str(i), nom_ar, d1_s, d2_s, "{:.2f}".format(row.Moyenne), mention, trend, prog_s])

    # Ligne moyenne classe — exactement len(headers) colonnes
    moy_s = "{:.2f}".format(moy_classe)
    moy_row = ["—"] * len(headers)
    moy_row[0] = "—"
    moy_row[1] = "MOYENNE CLASSE"
    # Trouver index de la colonne Moy./20
    try:
        moy_idx = headers.index("Moy./20")
        moy_row[moy_idx] = moy_s
    except Exception:
        moy_row[-3] = moy_s
    tdata.append(moy_row)
...
def _detect_format(df_raw):
    """Détecte le format MASSAR (2D+Act ou 3D+Act) depuis un DataFrame pandas brut."""
    for i, row in df_raw.iterrows():
        row_str = " ".join([str(v) for v in row if pd.notna(v)])
        if "الفرض الأول" in row_str or "إسم التلميذ" in row_str:
            # ── Détecter nb devoirs via ligne des marqueurs (#1#,#2#,#3#,#4#) ──
            # La ligne des marqueurs est 1 ligne AVANT le header
            nb_devoirs = 3  # défaut
            if i > 0:
                marqueurs_row = df_raw.iloc[i - 1]
                marqueurs_str = " ".join([str(v) for v in marqueurs_row if pd.notna(v)])
                # #3# présent = 3 devoirs, absent = 2 devoirs
                if "#3#" in marqueurs_str:
                    nb_devoirs = 3
                else:
                    nb_devoirs = 2

            # ── Positions colonnes selon nb_devoirs ──
            # 2D+Act : col6=D1, col8=D2, col10=Act
            # 3D+Act : col6=D1, col8=D2, col10=D3, col12=Act
            if nb_devoirs == 2:
                col_d1, col_d2, col_d3, col_act = 6, 8, None, 10
            else:
                col_d1, col_d2, col_d3, col_act = 6, 8, 10, 12
            ...
            for j in range(i + 2, len(df_raw)):
                ...
                try:
                    ...
                    if nom and isinstance(nom, str) and len(nom.strip()) > 1:
                        d1  = r.iloc[col_d1]  if len(r) > col_d1  else None
                        d2  = r.iloc[col_d2]  if len(r) > col_d2  else None
                        d3  = r.iloc[col_d3]  if (col_d3 and len(r) > col_d3) else None
                        act = r.iloc[col_act] if len(r) > col_act else None
...
if "nom_classe" not in st.session_state:
    st.session_state.nom_classe  = ""
    ...
    if missing:
        st.error("Colonnes introuvables. Vérifiez le format du fichier.")
            ...
            box_d1 = (
                ...
                '<div class="stat-value">' + "{:.2f}".format(moy_d1) + '</div></div>'
            )
            box_d2 = (
                '<div class="stat-box"><div class="stat-label">Moy. Devoir 2</div>'
                '<div class="stat-value">' + "{:.2f}".format(moy_d2) + '</div></div>'
            )
            box_d3 = (
                '<div class="stat-box"><div class="stat-label">Moy. Devoir 3</div>'
                '<div class="stat-value">' + "{:.2f}".format(moy_d3) + '</div></div>'
            ) if moy_d3 is not None else ""
            box_act = (
                '<div class="stat-box gold"><div class="stat-label">Moy. Activites</div>'
                '<div class="stat-value">' + "{:.2f}".format(moy_act) + '</div></div>'
            ) if moy_act is not None else ""
            html_row2 = (
                '<div class="stat-grid">'
                + box_d1 + box_d2 + box_d3 + box_act
                + '</div>'
            )
            ...
            with tab1:
                ...
                for _, row in df.sort_values("Moyenne", ascending=False).iterrows():
                    ...
                    d1_val  = row["Devoir1"]
                    d2_val  = row["Devoir2"]
                    d3_val  = row["Devoir3"] if (has_d3 and pd.notna(row.get("Devoir3"))) else None
                    act_val = row["Activites"] if ("Activites" in row.index and pd.notna(row["Activites"])) else None

                    # Progression : D3-D1 si 3 devoirs, D2-D1 si 2 devoirs
                    if has_d3 and d3_val is not None:
                        prog = float(d3_val) - float(d1_val)
                        prog_lbl = "Prog D1-D3 : "
                    else:
                        prog = float(d2_val) - float(d1_val)
                        prog_lbl = "Prog D1-D2 : "

                    # Vérifier que prog n'est pas NaN
                    if pd.isna(prog):
                        prog_str  = "N/A"
                        prog_color = "#888888"
                    else:
                    ...
                    if d3_val is not None:
                        trend_txt, trend_color = get_trend(float(d1_val), float(d2_val), float(d3_val))
                    else:
                        trend_txt, trend_color = get_trend(float(d1_val), float(d2_val))

                    moy_str = "{:.2f}".format(row["Moyenne"])
                    nom     = str(row["Etudiant"])
                    d1_str  = str(d1_val)
                    d2_str  = str(d2_val)
                    d3_str  = "{:.2f}".format(float(d3_val)) if d3_val is not None else None
                    act_str = "{:.2f}".format(float(act_val)) if act_val is not None else None
                    pbar    = progress_bar(row["Moyenne"], 20, trend_color)

                    # HTML parties
                    d3_part  = ' &nbsp;&rarr;&nbsp; D3: <b>' + d3_str + '</b>' if d3_str else ''
                    act_part = ' &nbsp;&nbsp; Act: <b style="color:#F9A825"> ' + act_str + '</b>' if act_str else ''
                    prog_lbl_html = '&nbsp;&nbsp; ' + prog_lbl
            ...
            with tab3:
                ...
                # Détection locale has_d3 / has_act pour le classement
                has_d3_r  = "Devoir3" in df_rank.columns and df_rank["Devoir3"].notna().any()
                has_act_r = "Activites" in df_rank.columns and df_rank["Activites"].notna().any()

                # Progression
                if has_d3_r:
                    prog_s = df_rank["Devoir3"] - df_rank["Devoir1"]
                else:
                    prog_s = df_rank["Devoir2"] - df_rank["Devoir1"]
                df_rank["Prog"] = prog_s.apply(
                    lambda x: ("+{:.1f}".format(x)) if x > 0 else "{:.1f}".format(x))

                # Tendance
                if has_d3_r:
                    df_rank["Tendance"] = df_rank.apply(
                        lambda r: get_trend(r["Devoir1"], r["Devoir2"],
                                            r["Devoir3"] if pd.notna(r["Devoir3"]) else r["Devoir2"])[0], axis=1)
                else:
                    df_rank["Tendance"] = df_rank.apply(
                        lambda r: get_trend(r["Devoir1"], r["Devoir2"])[0], axis=1)

                # En-têtes colonnes
                d3_th  = '<th style="padding:10px 6px">D3</th>'  if has_d3_r  else ""
                act_th = '<th style="padding:10px 6px">Act.</th>' if has_act_r else ""
                prog_th_label = "Prog D1-D3" if has_d3_r else "Prog D1-D2"
                rows_html = ""

                for _, r in df_rank.iterrows():
                    rang = int(r["Rang"])
                    if rang == 1:   medal = "1er"
                    elif rang == 2: medal = "2e"
                    elif rang == 3: medal = "3e"
                    else:           medal = str(rang)
                    ...
                    prog_val   = str(r["Prog"])
                    prog_color = "#2E7D32" if "+" in prog_val else "#C62828"
                    mention, _ = get_mention(moy)

                    # Cellule D3
                    if has_d3_r:
                        d3_v = r["Devoir3"]
                        d3_cell = '<td style="text-align:center">' + (str(d3_v) if pd.notna(d3_v) else "-") + '</td>'
                    else:
                        d3_cell = ""
                    ...
                    if has_act_r:
                        act_v = r["Activites"]
                        act_cell = '<td style="text-align:center;font-weight:700;color:#7B5800">' + (str(act_v) if pd.notna(act_v) else "-") + '</td>'
                    else:
                        act_cell = ""
                    ...
                    rows_html += (
                        ...
                        + d3_cell + act_cell
                        + '<td style="text-align:center;font-weight:900;color:#0D47A1">' + str(r["Moy."]) + '</td>'
                        '<td style="text-align:center"><span style="background:white;'
                ...
                table_html = (
                    ...
                    '<th style="padding:10px 6px">' + prog_th_label + '</th>'
            ...
            with tab4:
                ...
                with dl2:
                    ...
                    try:
                        pdf = make_pdf_bulletin(df, st.session_state.nom_classe)
